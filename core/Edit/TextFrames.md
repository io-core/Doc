...
```
 replace* = Texts.replace; insert* = Texts.insert; delete* = Texts.delete; unmark* = Texts.unmark;  (*message ids*)
    ...
    ## Constants:
```
 K = 100; len = 75;  (*assumed number of lines and average line length*)
...
```
 TBuf*, DelBuf: Texts.Buffer;
  ...
  PROCEDURE NewLine (F: Frame; ## Variables:
```
 L: Line);  (*reuse line from line pool if possible*)
...
```
 DW, DH, CL: INTEGER;
  BEGIN DW := Display.Width; DH := Display.Height; CL := DW;
    ...
    Display.CopyPatternClipped(Display.white, Display.updown, X-4, Y-4, 0, 0, 0, 0, Display.invert)
  END FlipSM;
    ## Variables:
```
 h, oldH: INTEGER;
...
```
 h: INTEGER;
  BEGIN h := Min(F.H - 2, 20);
    ...
    F.hasMark := on
  END Mark;
  PROCEDURE Width (## Variables:
```
 R: Texts.Reader; len: LONGINT): INTEGER;
    ## Variables:
```
 patadr, pos: LONGINT; ox, dx, x, y, w, h: INTEGER;
    ## Variables:
```
 patadr, pos: LONGINT; ox, dx, x, y, w, h: INTEGER;
  BEGIN pos := 0; ox := 0;
    ...
    RETURN ox
  END Width;
  PROCEDURE MeasureLine (## Variables:
```
 l: LineDesc; ## Variables:
```
 R: Texts.Reader);
    ## Variables:
```
 maxY, minY, len: INTEGER;
    ## Variables:
```
 maxY, minY, len: INTEGER;
  ...
  END MeasureLine;
  PROCEDURE PrepareLine (L: Line; ## Variables:
```
 R: Texts.Reader);  (*pass 1*)
  BEGIN MeasureLine(L^, R)
  END PrepareLine;
    ## Variables:
```
 R: Texts.Reader; X, Y, topY, botY: INTEGER; len: LONGINT);
    ## Variables:
```
 patadr, NX, dx, x, y, w, h, left, right, top, bot: INTEGER;
    ## Variables:
```
 patadr, NX, dx, x, y, w, h, left, right, top, bot: INTEGER;
  BEGIN NX := F.X + F.W - F.right; left := 0;
    ...
    L.eot := R.fnt = NIL; Texts.Read(R, nextCh)
  END DisplayLine;
  PROCEDURE Validate (T: Texts.Text; ## Variables:
```
 pos: LONGINT);
    ## Variables:
```
...
```
 R: Texts.Reader; L, l: Line;
  ...
  END ScrollUp;
    ## Variables:
```
 R: Texts.Reader; L, L0: Line; l: LineDesc;
      org, org0, q: LONGINT; dy, i, k: INTEGER;
      lines: ARRAY K OF LineDesc;  (*cache to prevent re-reading of lines*)
  BEGIN (*dY # 0*)
    IF F.trailer.next # F.trailer THEN
...
```
 R: Texts.Reader; L, L0: Line; l: LineDesc;
  ...
  BEGIN
    ...
    END
  END Show;
  PROCEDURE LocateLine (F: Frame; y: INTEGER; ## Variables:
```
 loc: Location);
    ## Variables:
```
 L: Line; org: LONGINT; cury: INTEGER;
    ## Variables:
```
 L: Line; org: LONGINT; cury: INTEGER;
  BEGIN org := F.org; L := F.trailer.next; cury := F.H - F.top + F.voff;
    ...
    loc.org := org; loc.lin := L; loc.y := cury - L.asr
  END LocateLine;
  PROCEDURE LocateString (F: Frame; x, y: INTEGER; ## Variables:
```
 loc: Location);
    ## Variables:
```
...
```
 R: Texts.Reader;
  ...
  BEGIN LocateLine(F, y, loc);
    ...
    loc.pos := bpos; loc.dx := ex - bx; loc.x := bx
  END LocateString;
  PROCEDURE LocateChar (F: Frame; x, y: INTEGER; ## Variables:
```
 loc: Location);
    ## Variables:
```
...
```
 R: Texts.Reader;
  ...
  BEGIN LocateLine(F, y, loc);
    ...
    loc.pos := pos; loc.dx := dx; loc.x := ox
  END LocateChar;
  PROCEDURE LocatePos (F: Frame; pos: LONGINT; ## Variables:
```
 loc: Location);
    ## Variables:
```
 T: Texts.Text; R: Texts.Reader; L: Line;
    ## Variables:
```
 T: Texts.Text; R: Texts.Reader; L: Line;
      org: LONGINT; cury: INTEGER;
...
```
 loc: Location;
  BEGIN LocateChar(F, X - F.X, Y - F.Y, loc); RETURN loc.pos
  END Pos;
  PROCEDURE TrackCaret* (F: Frame; X, Y: INTEGER; ## Variables:
```
 keysum: SET);
    ## Variables:
```
 loc: Location; keys: SET;
    ## Variables:
```
 loc: Location; keys: SET;
  BEGIN
    ...
    END
  END TrackCaret;
  PROCEDURE FlipSelection (F: Frame; ## Variables:
```
 beg, end: Location);
    ## Variables:
```
 L: Line; Y: INTEGER; org: LONGINT; beg0: Location;
    ## Variables:
```
 L: Line; Y: INTEGER; org: LONGINT; beg0: Location;
  BEGIN
    ...
    END
  END FlipSelection;
  PROCEDURE TrackSelection* (F: Frame; X, Y: INTEGER; ## Variables:
```
 keysum: SET);
    ## Variables:
```
 loc, L, R: Location; keys: SET; start, prev: LONGINT; topY, botY: INTEGER;
    ## Variables:
```
 loc, L, R: Location; keys: SET; start, prev: LONGINT; topY, botY: INTEGER;
  BEGIN
    ...
    END
  END TrackSelection;
  PROCEDURE TrackLine* (F: Frame; X, Y: INTEGER; ## Variables:
```
 org: LONGINT; ## Variables:
```
 keysum: SET);
    ## Variables:
```
 old, new: Location; keys: SET;
    ## Variables:
```
 old, new: Location; keys: SET;
  BEGIN
    ...
    END
  END TrackLine;
  PROCEDURE TrackWord* (F: Frame; X, Y: INTEGER; ## Variables:
```
 pos: LONGINT; ## Variables:
```
 keysum: SET);
    ## Variables:
```
...
```
 L: Line;
  BEGIN NEW(L);
    ...
    F.pool := NIL; F.voff := 0
  END Open;
  PROCEDURE Copy* (F: Frame; ## Variables:
```
 F1: Frame);
  BEGIN NEW(F1);
    Open(F1, F.handle, F.text, F.org, F.col, F.left, F.right, F.top, F.bot, F.lsp)
...
```
 beg, end, time: LONGINT);
  BEGIN
    ...
    END
  END GetSelection;
  PROCEDURE Update* (F: Frame; ## Variables:
```
 M: UpdateMsg);
  BEGIN (*F.text = M.text*) SetChangeMark(F, FALSE);
    RemoveMarks(F); Oberon.RemoveMarks(F.X, F.Y, F.W, F.H);
