...
```
 Q, q, p: Piece;
  ...
  END Load;
    ## Variables:
```
 f: Files.File; R: Files.Rider; Q, q: Piece;
...
```
 f: Files.File; w: Files.Rider;
  ...
  END Close;
  PROCEDURE FindPiece (T: Text; pos: LONGINT; ## Variables:
```
...
```
 p: Piece; porg: LONGINT;
  BEGIN p := T.pce; porg := T.org;
    ...
    pce := p; org := porg
  END FindPiece;
  PROCEDURE SplitPiece (p: Piece; off: LONGINT; ## Variables:
```
 pr: Piece);
    ## Variables:
```
...
```
 Q, q, p: Piece;
  ...
  END Copy;
    ## Variables:
```
 pl, pr, p, qb, qe: Piece; org, end: LONGINT;
  BEGIN
  ...
  END Insert;
    ## Variables:
```
 pb, pe, pbr, per: Piece; orgb, orge: LONGINT;
  BEGIN
    IF end > T.len THEN end := T.len END;
...
```
 pb, pe, p: Piece; org: LONGINT;
  ...
  END ChangeLooks;
  PROCEDURE Attributes*(T: Text; pos: LONGINT; ## Variables:
```
 fnt: Fonts.Font; ## Variables:
```
 col, voff: INTEGER);
    ## Variables:
```
...
```
 p: Piece; org: LONGINT;
  ...
  END Attributes;
  PROCEDURE OpenReader* (## Variables:
```
 R: Reader; T: Text; pos: LONGINT);
    ## Variables:
```
 p: Piece; org: LONGINT;
    ## Variables:
...
```
 ch: CHAR);
  BEGIN Files.Read(R.rider, ch);
    ...
    END
  END Read;
  PROCEDURE Pos* (## Variables:
```
 R: Reader): LONGINT;
  BEGIN RETURN R.org + R.off
  END Pos;  
  PROCEDURE OpenScanner* (## Variables:
```
 S: Scanner; T: Text; pos: LONGINT);
...
```
 ch: CHAR;
  ...
  BEGIN ch := S.nextCh; i := 0;
    ...
    S.nextCh := ch
  END Scan;
  PROCEDURE OpenWriter* (## Variables:
```
 W: Writer);
  BEGIN NEW(W.buf);
    OpenBuf(W.buf); W.fnt := Fonts.Default; W.col := 15; W.voff := 0;
    Files.Set(W.rider, Files.New(""), 0)
  END OpenWriter;
  PROCEDURE SetFont* (## Variables:
```
 W: Writer; fnt: Fonts.Font);
  BEGIN W.fnt := fnt
  END SetFont;
  PROCEDURE SetColor* (## Variables:
```
 W: Writer; col: INTEGER);
  BEGIN W.col := col
  END SetColor;
  PROCEDURE SetOffset* (## Variables:
```
 W: Writer; voff: INTEGER);
  BEGIN W.voff := voff
  END SetOffset;
...
```
 p: Piece;
  BEGIN
    ...
    INC(W.buf.last.len); INC(W.buf.len)
  END Write;
  PROCEDURE WriteLn* (## Variables:
```
 W: Writer);
  BEGIN Write(W, CR)
  END WriteLn;
  PROCEDURE WriteString* (## Variables:
```
 W: Writer; s: ARRAY OF CHAR);
    ## Variables:
```
 i: INTEGER;
    ## Variables:
```
 i: INTEGER;
  BEGIN i := 0;
    WHILE s[i] >= " " DO Write(W, s[i]); INC(i) END
  END WriteString;
  PROCEDURE WriteInt* (## Variables:
```
 W: Writer; x, n: LONGINT);
    ## Variables:
```
...
```
 i: INTEGER; y: LONGINT;
  ...
  BEGIN i := 0; Write(W, " ");
    ...
    REPEAT DEC(i); Write(W, a[i]) UNTIL i = 0
  END WriteHex;
 PROCEDURE WriteReal* (## Variables:
```
 W: Writer; x: REAL; n: INTEGER);
    ## Variables:
```
 e, i, k, m: INTEGER;
    ## Variables:
```
 e, i, k, m: INTEGER;
  ...
  BEGIN e := ASR(ORD(x), 23) MOD 100H;  (*binary exponent*)
    ...
    END
  END WriteReal;
  PROCEDURE WriteRealFix* (## Variables:
```
 W: Writer; x: REAL; n, k: INTEGER);
    ## Variables:
```
 i, m: INTEGER; neg: BOOLEAN;
    ## Variables:
```
 i, m: INTEGER; neg: BOOLEAN;
      d: ARRAY 12 OF CHAR;
  BEGIN
    ...
    END
  END WriteRealFix;
  PROCEDURE WritePair(## Variables:
```
 W: Writer; ch: CHAR; x: LONGINT);
  BEGIN Write(W, ch);
    Write(W, CHR(x DIV 10 + 30H)); Write(W, CHR(x MOD 10 + 30H))
  END WritePair;
  PROCEDURE WriteClock* (## Variables:
```
 W: Writer; d: LONGINT);
  BEGIN
    WritePair(W, " ", d DIV 20000H MOD 20H);   (*day*)
