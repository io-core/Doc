...
```
 V: Viewers.Viewer; X, Y: INTEGER;
  BEGIN Oberon.AllocateSystemViewer(0, X, Y);
    ...
    V := MenuViewers.New(
      TextFrames.NewMenu(SystemLog, LogMenu),
      TextFrames.NewText(Oberon.Log, 0), TextFrames.menuH, X, Y)
  END OpenLog;
    ## Variables:
```
 T: Texts.Text; F: Display.Frame; buf: Texts.Buffer;
...
```
 D: Viewers.DisplayArea; V: Viewers.Viewer;
  ...
  END CloseTrack;
    ## Variables:
```
 V: Viewers.Viewer; curDisplay: Viewers.DisplayArea; M: Viewers.ViewerMsg;
  ...
  END Recall;
    ## Variables:
```
 V: Viewers.Viewer; M: Viewers.ViewerMsg;
  ...
  END RecallFocus;
    ## Variables:
```
 V, V1: Viewers.Viewer; M: Oberon.CopyMsg; N: Viewers.ViewerMsg;
  ...
  END Copy;
    ## Variables:
```
 D: Viewers.DisplayArea; V, V1: Viewers.Viewer; M: Oberon.CopyMsg; N: Viewers.ViewerMsg;
...
```
 D: Viewers.DisplayArea; V: Viewers.Viewer;
  ...
  END Spread;
  PROCEDURE MakeDisplayName(prefix: ARRAY OF CHAR; id: INTEGER; ## Variables:
```
 name: ARRAY OF CHAR);  (*0 <= id < 10^5*)
    ## Variables:
```
 i, j: INTEGER; buf: ARRAY 5 OF INTEGER;
    ## Variables:
```
 i, j: INTEGER; buf: ARRAY 5 OF INTEGER;
  BEGIN i := 0; j := 0;
...
```
 S: Texts.Scanner; prev: INTEGER;
  ...
  END OpenDisplay;
    ## Variables:
```
 D: Viewers.DisplayArea; V, V1: Viewers.Viewer;
  ...
  END Clone;
    ## Variables:
```
 V: Viewers.Viewer; curDisplay: Viewers.DisplayArea;
  ...
  END ExtendDisplay;
    ## Variables:
```
 D: Viewers.DisplayArea; S: Texts.Scanner; prev, id, hint: INTEGER;
  BEGIN Texts.OpenScanner(S, Oberon.Par.text, Oberon.Par.pos); Texts.Scan(S);
...
```
 D: Viewers.Viewer;
  ...
  END PrevDisplay;
    ## Variables:
```
 D: Viewers.DisplayArea; S: Texts.Scanner;
  BEGIN Texts.OpenScanner(S, Oberon.Par.text, Oberon.Par.pos); Texts.Scan(S);
...
```
 S: Texts.Scanner; on, this, clients, imports: BOOLEAN; handle: Handler);  (*modules*)
    ## Variables:
```
 T: Texts.Text;
    ## Variables:
...
```
 mod: Modules.Module;
  BEGIN mod := Modules.root;
    ...
    END
  END WriteSelection;
  PROCEDURE WriteRef(src, dst: LONGINT; s: ARRAY OF CHAR; ## Variables:
```
 continue: BOOLEAN): INTEGER;  (*write one reference*)
    ## Variables:
```
 mod: Modules.Module; i: INTEGER;
    ## Variables:
```
 mod: Modules.Module; i: INTEGER;
  BEGIN i := 0;
...
```
 resTyp, resPtr, resPvr: INTEGER;
  BEGIN Oberon.ClearPar;  (*exclude viewers that are reachable only via Oberon.Par.vwr*)
    M := NIL; ref := Modules.dyntypes; Modules.FindDynamicReferences(WriteRef, NIL, NIL, resTyp, resPtr, resPvr, FALSE);
    M := NIL; ref := Modules.dynptrs; Modules.FindDynamicReferences(NIL, WriteRef, NIL, resTyp, resPtr, resPvr, FALSE);
    M := NIL; ref := Modules.dynpvrs; Modules.FindDynamicReferences(NIL, NIL, WriteRef, resTyp, resPtr, resPvr, FALSE);
    M := NIL; ref := Modules.statptrs; Modules.FindStaticReferences(WriteRef, NIL, resPtr, resPvr);
    M := NIL; ref := Modules.statpvrs; Modules.FindStaticReferences(NIL, WriteRef, resPtr, resPvr);
    Texts.Append(Oberon.Log, W.buf)
  END WriteReferences;
    ## Variables:
```
 S: Texts.Scanner;
  BEGIN Texts.WriteString(W, "System.ShowReferences"); EndLine;
    Modules.Deselect; Select(S, TRUE, TRUE, FALSE, FALSE, ShowRefs)
  END ShowReferences;
    ## Variables:
```
 S: Texts.Scanner;
  BEGIN Texts.WriteString(W, "System.ShowGroupReferences");
    Modules.Deselect; Select(S, TRUE, TRUE, FALSE, FALSE, WriteName); EndLine;
    IF Modules.NofSelected > 0 THEN WriteReferences; Modules.Deselect END
  END ShowGroupReferences;
    ## Variables:
```
 S: Texts.Scanner;
  ...
  END Show;
  PROCEDURE WriteClient(mod, imp: Modules.Module; ## Variables:
```
 continue: BOOLEAN): INTEGER;
  ...
  END WriteClient;
    ## Variables:
```
 S: Texts.Scanner; hide: BOOLEAN; res: INTEGER;
  BEGIN Modules.Deselect; Select(S, TRUE, this, clients, imports, WriteName); EndLine;
    ...
    IF Modules.NofSelected > 0 THEN Texts.Write(W, " "); WriteSelection; Modules.FreeSelection(hide);
      ...
      ELSE
        ...
        END ;
        EndLine; WriteReferences
      END ;
      Modules.Deselect; Oberon.Collect(0)
    END
  END FreeModules;
  PROCEDURE List(name: FileDir.FileName; adr: LONGINT; ## Variables:
```
 cont: BOOLEAN);
    ## Variables:
```
 i0, i, j0, j, length, date: INTEGER;
    ## Variables:
```
 i0, i, j0, j, length, date: INTEGER;
  BEGIN i := 0;
    ...
    IF (name[i0] = 0X) & (pat[j0] = 0X) THEN (*found*)
      ...
      IF pat[j0+1] = "!" THEN (*option*)
        ...
        Texts.Write(W, TAB); Texts.WriteClock(W, date); Texts.WriteInt(W, length, 8); Texts.WriteHex(W, adr)
      END ;
      Texts.WriteLn(W)
    END
  END List;
...
```
 T: Texts.Text;
  ...
  BEGIN T := TextFrames.Text("");
    ...
    WHILE M # NIL DO
      IF M.name[0] # 0X THEN
        Texts.WriteString(W, M.name); Texts.WriteInt(W, M.num, 4); Texts.Write(W, TAB);
...
```
 fnt: Fonts.Font;
  BEGIN Texts.WriteString(W, "System.ShowFonts"); Texts.WriteLn(W); fnt := Fonts.root;
    WHILE fnt # NIL DO
      Texts.Write(W, TAB); Texts.WriteString(W, fnt.name); Texts.WriteLn(W); fnt := fnt.next
    END ;
    Texts.Append(Oberon.Log, W.buf)
  END ShowFonts;
    ## Variables:
...
```
 a: INTEGER; b, c, d: INTEGER);
    ## Variables:
```
 u, v, w: INTEGER; mod: Modules.Module;
    ## Variables:
```
 u, v, w: INTEGER; mod: Modules.Module;
  ...
  END Trap;
    ## Variables:
```
 u: INTEGER; mod: Modules.Module;
  BEGIN u := SYSTEM.REG(LNK);  (*return address deposited in register LNK by the abort (=BL 0) instruction*)
  ...
  END Abort;
    ## Variables:
```
 v, w, pos: INTEGER; mod: Modules.Module;
  BEGIN
