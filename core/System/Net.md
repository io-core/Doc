...
```
 PakSize = 512;
  ...
  PROCEDURE AppendW(s: LONGINT; VAR d: ARRAY OF CHAR; n: INTEGER; VAR k: INTEGER);
    VAR i: INTEGER;
  BEGIN i := 0;
    REPEAT d[k] := CHR(s MOD 100H); s := s DIV 100H; INC(i); INC(k) UNTIL i = n
  END AppendW;
  ...
  BEGIN Files.Set(R, F, 0); len := 0; seqno := 0;
    REPEAT k := 0;
      REPEAT Files.ReadByte(R, x);
        IF ~R.eof THEN buf[k] := CHR(x); INC(k) END
  ...
  BEGIN
    ...
    IF head1.valid THEN
      ...
      ELSIF head1.typ = MSG THEN i := 0;
        WHILE i < head1.len DO SCC.Receive(x); Texts.Write(W, CHR(x)); INC(i) END ;
      ...
      ELSIF head1.typ = TIM THEN PickQ(newclock); PickS(Id); PickQ(pw);
        ...
        IF ~protected & (Id[0] # 0X) & (ABS(pw - clock) > 10) THEN
          ...
          Texts.WriteString(W, Id); Texts.WriteString(W, " changed System.Date");
  ...
  BEGIN protected := TRUE; partner[0] := 0X; SCC.Start(TRUE);
    ...
    Texts.WriteString(W, " Server started as "); Texts.WriteString(W, Oberon.User);
    Texts.WriteLn(W); Texts.Append(Oberon.Log, W.buf)
  END StartServer;
...
```
 time: LONGINT;
  BEGIN time := Oberon.Time() + timeout;
    ...
    UNTIL head1.valid OR (head1.typ = 0FFH)
  END ReceiveHead;
  PROCEDURE FindPartner(name: ARRAY OF CHAR; ## Variables:
```
 res: INTEGER);
    ## Variables:
```
 time: LONGINT; k: INTEGER; Id: ARRAY 8 OF CHAR;
    ## Variables:
```
 time: LONGINT; k: INTEGER; Id: ARRAY 8 OF CHAR;
  BEGIN SCC.Skip(SCC.Available()); res := 0; k := 0;
    ...
    END
  END FindPartner;
  PROCEDURE AppendS(s: ARRAY OF CHAR; ## Variables:
```
 d: ARRAY OF CHAR; ## Variables:
```
 k: INTEGER);
    ## Variables:
```
...
```
 i: INTEGER; ch: CHAR;
  BEGIN i := 0;
    REPEAT ch := s[i]; d[k] := ch; INC(i); INC(k) UNTIL ch = 0X
  END AppendS;
  PROCEDURE AppendW(s: LONGINT; ## Variables:
```
 d: ARRAY OF CHAR; n: INTEGER; ## Variables:
```
 k: INTEGER);
    ## Variables:
```
...
```
 k, seqno: INTEGER;
  ...
  BEGIN Files.Set(R, F, 0); len := 0; seqno := 0;
    ...
    Texts.WriteInt(W, len, 7)
  END SendData;
  PROCEDURE ReceiveData(F: Files.File; ## Variables:
```
 done: BOOLEAN);
    ## Variables:
```
 k, retry, seqno: INTEGER;
    ## Variables:
```
 k, retry, seqno: INTEGER;
     x: BYTE;
...
```
 i: INTEGER;
  ...
  BEGIN
    ...
    END
  END Serve;
  PROCEDURE GetPar1(## Variables:
```
 S: Texts.Scanner);
  BEGIN Texts.OpenScanner(S, Oberon.Par.text, Oberon.Par.pos); Texts.Scan(S)
  END GetPar1;
  PROCEDURE GetPar(## Variables:
```
