...
```
 root: INTEGER (*File*);  (*list of open files*)
        ## Variables:
```
 name: FileDir.FileName; ## Variables:
```
 res: INTEGER);
    ## Variables:
```
    ...
    IF res = 0 THEN
      ...
      IF header # 0 THEN
        ...
        IF f = NIL THEN (*file not yet present*)
          ...
          Disk.GetSector(header, buf.data); ASSERT(F.mark = FileDir.HeaderMark);
...
```
 i, res: INTEGER;
  ...
  BEGIN f := NIL; Check(name, namebuf, res);
    ...
    RETURN f
  END New;
  PROCEDURE UpdateHeader(f: File; ## Variables:
```
 F: FileDir.FileHeader);
    ## Variables:
```
 k: INTEGER;
    ## Variables:
    ...
    IF f # NIL THEN a := f.aleng + 1; f.aleng := 0; f.bleng := HS;
      ...
      ELSE i := STS; DEC(a, i); j := (a-1) MOD XS; k := (a-1) DIV XS;
        WHILE k >= 0 DO
          ...
          REPEAT DEC(j); Disk.FreeSector(ind[j]) UNTIL j = 0;
          Disk.FreeSector(f.ext[k].adr); j := XS; DEC(k)
        END
      END ;
      REPEAT DEC(i); Disk.FreeSector(f.sec[i]) UNTIL i = 0
    END
  END Purge;
  PROCEDURE Delete*(name: ARRAY OF CHAR; ## Variables:
```
 res: INTEGER);
    ## Variables:
```
...
```
 adr: DiskAdr;
  ...
  BEGIN Check(name, namebuf, res);
    ...
    END
  END Delete;
  PROCEDURE Rename*(old, new: ARRAY OF CHAR; ## Variables:
```
 res: INTEGER);
    ## Variables:
```
...
```
 r: Rider): INTEGER;
  ...
  END Pos;
  PROCEDURE Base*(## Variables:
```
 r: Rider): File;
  BEGIN RETURN r.file
  END Base;
  PROCEDURE ReadByte*(## Variables:
```
 r: Rider; ## Variables:
```
 x: BYTE);
...
```
 buf: Buffer;
  BEGIN
    ...
    END
  END ReadByte;
  PROCEDURE ReadBytes*(## Variables:
```
 r: Rider; ## Variables:
```
 x: ARRAY OF BYTE; n: INTEGER);
    ## Variables:
```
 i: INTEGER;
    ## Variables:
...
```
 n: INTEGER;
  ...
  END ReadReal;
  PROCEDURE ReadString*(## Variables:
```
 R: Rider; ## Variables:
```
 x: ARRAY OF CHAR);
...
```
 f: File; buf: Buffer;
  ...
  END WriteByte;
  PROCEDURE WriteBytes*(## Variables:
```
 r: Rider; x: ARRAY OF BYTE; n: INTEGER);
    ## Variables:
```
 i: INTEGER;
    ## Variables:
