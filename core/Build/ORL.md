...
```
 FName: ARRAY OF CHAR; name, ext: ARRAY OF CHAR);
    ## Variables:
```
 i, j: INTEGER;
    ## Variables:
...
```
 i: INTEGER; ch: CHAR;
  BEGIN ch := s[0]; res := 1; i := 1;
    ...
    END
  END Check;
  PROCEDURE LinkOne(name: ARRAY OF CHAR; ## Variables:
```
 newmod: Module);
    ...
    IF mod = NIL THEN (*link*)
      ...
      IF res = noerr THEN (*fixup of BL*)
        ...
        WHILE adr # mod.code DO
          ...
          END ;
          SYSTEM.PUT(adr, MOV+U+B + pno*C24 + (offset - Start) DIV C16);  (*mark as fixed up by setting the B bit*)
...
```
 p, i, j, x, y, im: INTEGER; M: Module;
  ...
  END RelocateLoaded;
    ## Variables:
```
 i, start, filelen, destadr, fprint: INTEGER; b: BYTE;
  ...
  END Relocate;
    ## Variables:
```
 i, start, filelen, fprint: INTEGER; b: BYTE;
