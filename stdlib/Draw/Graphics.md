...
```
        draw*, change*: PROCEDURE (obj: Object; ## Variables:
```
 msg: Msg);
        selectable*: PROCEDURE (obj: Object; x, y: INTEGER): BOOLEAN;
        read*: PROCEDURE (obj: Object; ## Variables:
```
...
```
 C: Context);
        write*: PROCEDURE (obj: Object; cno: INTEGER; ## Variables:
```
 R: Files.Rider; ## Variables:
```
 C: Context);
...
```
 width*, res*: INTEGER;
    ...
    GetLib0: PROCEDURE (name: ARRAY OF CHAR; replace: BOOLEAN; ## Variables:
```
 Lib: Library);
    GetLib0: PROCEDURE (name: ARRAY OF CHAR; replace: BOOLEAN; ## Variables:
```
...
```
 obj: Object; t: INTEGER;
  ...
  END SelectArea;
  PROCEDURE Draw*(G: Graph; ## Variables:
```
 M: Msg);
    ## Variables:
```
...
```
 obj: Object;
  ...
  END Draw;
    ## Variables:
```
 obj: Object; tag: INTEGER;
  BEGIN obj := G.first;
    WHILE obj # NIL DO
      Texts.Write(XW, TAB); Texts.WriteHex(XW, ORD(obj)); Texts.Write(XW, TAB);
...
```
 obj: Object;
  ...
  END Deselect;
  PROCEDURE DrawSel*(G: Graph; ## Variables:
```
 M: Msg);
    ## Variables:
```
...
```
 obj: Object;
  BEGIN obj := G.first;
    ...
    END
  END DrawSel;
  PROCEDURE Change*(G: Graph; ## Variables:
```
 M: Msg);
    ## Variables:
```
...
```
 obj: Object;
  ...
  END Change;
    ## Variables:
```
 obj, ob0: Object; x0, x1, y0, y1: INTEGER;
  BEGIN obj := G.first; G.changed := TRUE;
    WHILE obj # NIL DO
...
```
 fno: INTEGER;
  BEGIN fno := 0;
    ...
    RETURN fno
  END FontNo;
  PROCEDURE StoreElems(## Variables:
```
 W: Files.Rider; ## Variables:
```
...
```
 cno: INTEGER;
  ...
  END StoreElems;
  PROCEDURE Store*(G: Graph; ## Variables:
```
 W: Files.Rider);
    ## Variables:
```
...
```
 C: Context;
  ...
  END Store;
  PROCEDURE WriteObj*(## Variables:
```
 W: Files.Rider; cno: INTEGER; obj: Object);
  ...
  END WriteObj;
    ## Variables:
```
 F: Files.File; W: Files.Rider; C: Context;
...
```
 obj: Object;
  ...
  END Print;
  PROCEDURE GetClass*(module, allocator: ARRAY OF CHAR; ## Variables:
```
 com: Modules.Command);
    ## Variables:
```
...
```
 mod: Modules.Module;
  BEGIN Modules.Load(module, mod);
    ...
    END
  END GetClass;
  PROCEDURE Font*(## Variables:
```
 R: Files.Rider; ## Variables:
```
 C: Context): Fonts.Font;
    ## Variables:
```
 fno: BYTE;
    ## Variables:
```
 fno: BYTE;
  ...
  END Font;
  PROCEDURE ReadObj(## Variables:
```
 R: Files.Rider; obj: Object);
    ## Variables:
```
 xy, wh: INTEGER; dmy: BYTE;
    ## Variables:
```
 xy, wh: INTEGER; dmy: BYTE;
  ...
  BEGIN obj := NIL; Files.ReadByte(R, cno);
    WHILE ~R.eof & (cno < 255) DO
      IF cno = 0 THEN
        ...
        IF m = 0 THEN fnt := Fonts.Load(name); C.font[n] := fnt
    ...
    new := NIL; fobj := obj
  END LoadElems;
  PROCEDURE Load*(G: Graph; ## Variables:
```
 R: Files.Rider);
    ## Variables:
```
...
```
 tag: CHAR;
  ...
  BEGIN G.first := NIL; G.sel := NIL; G.time := 0; G.changed := FALSE; F := Files.Old(name);
    ...
    END
  END Open;
  PROCEDURE GetLib*(name: ARRAY OF CHAR; replace: BOOLEAN; ## Variables:
```
 Lib: Library);
    ## Variables:
```
 i, wh: INTEGER; ch: CHAR;
    ## Variables:
```
 i, wh: INTEGER; ch: CHAR;
...
```
 mh: MacHead;
  BEGIN mh := L.first;
    ...
    RETURN mh
  END ThisMac;
  PROCEDURE DrawMac*(mh: MacHead; ## Variables:
```
 M: Msg);
    ## Variables:
```
...
```
 obj: Object;
  BEGIN obj := mh.first;
    ...
    new := NIL
  END OpenMac;
  PROCEDURE MakeMac*(G: Graph; ## Variables:
```
 head: MacHead);
    ## Variables:
```
...
```
 x0, y0, x1, y1: INTEGER;
  ...
  BEGIN obj := G.first; last := NIL; x0 := 1024; x1 := 0; y0 := 1024; y1 := 0;
      ...
      new := NIL; head := mh
  END MakeMac;
  PROCEDURE InsertMac*(mh: MacHead; L: Library; ## Variables:
```
 new: BOOLEAN);
    ## Variables:
```
...
```
 line: Line;
  ...
  END NewLine;
  PROCEDURE ChangeLine(obj: Object; ## Variables:
```
 M: Msg);
  BEGIN
    CASE M OF
    ...
    END
  END ChangeLine;
  PROCEDURE ReadLine(obj: Object; ## Variables:
```
 R: Files.Rider; ## Variables:
```
 C: Context);
  BEGIN
  END ReadLine;
  PROCEDURE WriteLine(obj: Object; cno: INTEGER; ## Variables:
```
 W: Files.Rider; ## Variables:
```
 C: Context);
...
```
 cap: Caption;
  ...
  END NewCaption;
    ## Variables:
```
 ch: CHAR; R: Texts.Reader;
  BEGIN
    dst.x := src.x; dst.y := src.y; dst.w := src.w; dst.h := src.h; dst.col := src.col;
    dst(Caption).pos := T.len + 1; dst(Caption).len := src(Caption).len;
    Texts.Write(TW, CR); Texts.OpenReader(R, T, src(Caption).pos);
    Texts.Read(R, ch); TW.fnt := R.fnt;
    WHILE ch > CR DO Texts.Write(TW, ch); Texts.Read(R, ch) END ;
    Texts.Append(T, TW.buf)
  END CopyCaption;
  PROCEDURE ChangeCaption(obj: Object;  ## Variables:
```
 M: Msg);
    ## Variables:
```
 dx, x1, dy, y1, w, w1, h1, len: INTEGER;
    ## Variables:
```
 dx, x1, dy, y1, w, w1, h1, len: INTEGER;
      pos: LONGINT;
  ...
  BEGIN
    ...
    END
  END ChangeCaption;
  PROCEDURE ReadCaption(obj: Object; ## Variables:
```
 R: Files.Rider; ## Variables:
```
 C: Context);
    ## Variables:
```
 ch: CHAR; fno: BYTE; len: INTEGER;
    ## Variables:
```
 ch: CHAR; fno: BYTE; len: INTEGER;
  BEGIN obj(Caption).pos := T.len + 1; Texts.Write(TW, CR);
    Files.ReadByte(R, fno); TW.fnt := C.font[fno]; len := 0; Files.Read(R, ch);
    WHILE ch > CR DO Texts.Write(TW, ch); INC(len); Files.Read(R, ch) END ;
    obj(Caption).len := len; Texts.Append(T, TW.buf)
  END ReadCaption;
  PROCEDURE WriteCaption(obj: Object; cno: INTEGER; ## Variables:
```
 W: Files.Rider; ## Variables:
```
 C: Context);
 ...
 ch: CHAR; fno: BYTE;
  ...
  BEGIN
    IF obj(Caption).len > 0 THEN
      ...
      WHILE ch > CR DO  Files.Write(W, ch); Texts.Read(TR, ch) END ;
      Files.Write(W, 0X)
    END
  END WriteCaption;
    ## Variables:
...
```
 mac: Macro;
  ...
  END NewMacro;
  PROCEDURE ChangeMacro(obj: Object; ## Variables:
```
 M: Msg);
  BEGIN
    CASE M OF ColorMsg: obj.col := M.col END
  END ChangeMacro;
  PROCEDURE ReadMacro(obj: Object; ## Variables:
```
 R: Files.Rider; ## Variables:
```
 C: Context);
    ## Variables:
```
 lno: BYTE; name: ARRAY 32 OF CHAR;
    ## Variables:
```
 lno: BYTE; name: ARRAY 32 OF CHAR;
  ...
  END ReadMacro;
  PROCEDURE WriteMacro(obj: Object; cno: INTEGER; ## Variables:
```
 W1: Files.Rider; ## Variables:
```
 C: Context);
    ## Variables:
```
...
```
 elem: Object; mh: MacHead;
  BEGIN mh := obj(Macro).mac;
    ...
    END
  END PrintMacro; *)
  PROCEDURE InstallDrawMethods*(drawLine, drawCaption, drawMacro: PROCEDURE (obj: Object; ## Variables:
```
 msg: Msg));
