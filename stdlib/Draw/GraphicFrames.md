...
```
 w1, h1: INTEGER; DM: DispMsg;
  ...
  END InsertChar;
    ## Variables:
```
 w1, h1: INTEGER; ch: CHAR; pos: LONGINT;
...
```
 obj: Graphics.Object;
  BEGIN obj := F.graph.sel;
    ...
    END
  END GetSelection;
  PROCEDURE Handle*(G: Display.Frame; ## Variables:
```
 M: Display.FrameMsg);
    ## Variables:
```
 ...
 x, y, h: INTEGER;
  ...
  BEGIN
    CASE G OF Frame:
      ...
      | Viewers.ViewerMsg: G.Y := M.Y; G.H := M.H; Restore(G)
      END
    END
  END Handle;
  PROCEDURE DrawLine(obj: Graphics.Object; ## Variables:
```
 M: Graphics.Msg);
...
```
 x, y, w, h, col: INTEGER; f: Frame;
  BEGIN
    ...
    END
  END DrawLine;
  PROCEDURE DrawCaption(obj: Graphics.Object; ## Variables:
```
 M: Graphics.Msg);
    ## Variables:
```
 x, y, dx, x0, x1, y0, y1, w, h, w1, h1, col: INTEGER;
    ## Variables:
```
 x, y, dx, x0, x1, y0, y1, w, h, w1, h1, col: INTEGER;
      f: Frame;
  ...
  BEGIN
    ...
    END
  END DrawCaption;
  PROCEDURE DrawMacro(obj: Graphics.Object; ## Variables:
```
 M: Graphics.Msg);
    ## Variables:
```
