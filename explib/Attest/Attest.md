...
```
 S: Texts.Scanner;
  ...
  END SetWidth;
    ## Variables:
```
 S: Texts.Scanner; CM: KeySafes.ColorMsg;
  BEGIN GetArg(S);
    IF S.class = Texts.Int THEN
      CM.col := S.i MOD 16; KeySafeFrames.Change(KeySafeFrames.Selected(), CM)
    END
  END ChangeColor;
    ## Variables:
```
 S: Texts.Scanner; WM: KeySafes.WidMsg;
  BEGIN GetArg(S);
    IF S.class = Texts.Int THEN
      WM.w := S.i; KeySafeFrames.Change(KeySafeFrames.Selected(), WM)
    END
  END ChangeWidth;
    ## Variables:
```
 S: Texts.Scanner; FM: KeySafes.FontMsg;
  BEGIN GetArg(S);
    IF S.class = Texts.Name THEN
      FM.fnt := Fonts.This(S.s);
      IF FM.fnt # NIL THEN KeySafeFrames.Change(KeySafeFrames.Selected(), FM) END
  ...
  END ChangeFont;
    ## Variables:
```
 v: Viewers.Viewer; G: KeySafeFrames.Frame;
  BEGIN
    IF Oberon.Par.frame = Oberon.Par.vwr.dsc THEN v := Oberon.Par.vwr
    ELSE v := Oberon.MarkedViewer()
    END ;
